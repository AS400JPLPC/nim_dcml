...
proc rem*(a, b: DecimalType) =
  ...
  var status: uint32
  mpd_qrem(a[], a[], b[], CTX_ADDR, addr status)

proc fma*(a, b, c: DecimalType) =
  ## Fused multiplication-addition, returns a * b + c
  var status: uint32
  mpd_qfma(a[], a[], b[], c[], CTX_ADDR, addr status)
...
proc abs*(a: DecimalType) =
  ...
  mpd_qabs(a[], a[], CTX_ADDR, addr status)
...
proc floor*(a: DecimalType) =
  ...
  var status: uint32
  mpd_qfloor(a[], a[], CTX_ADDR, addr status)

proc ceil*(a: DecimalType) =
  ## Return the nearest integer towards +infinity
  var status: uint32
  mpd_qceil(a[], a[], CTX_ADDR, addr status)
...
proc aRound*(a: DecimalType; iScale:int )=
  ...
  if iScale > 0 :
    ...
    mpd_round_to_intx(dVal[], dVal[], CTX_CTRL)

    sVal = $dVal
    i = sVal.len - 1

    dVal /= 10
    mpd_trunc(dVal[], dVal[], CTX_CTRL)
    if sVal[i] > '4' : 
      dVal += 1
    for i in 1..iScale :
      dVal /= 10
...
proc Valide*(a: DecimalType) =
  ...
  if iScale != 0 :
    if dVal == a :
      ...
      dVal = newDecimal(sEntier)
    else :
      dVal = a
      for i in 1..iScale :
        dVal *= 10
      mpd_trunc(dVal[], dVal[], CTX_ADDR)
      for i in 1..iScale :
        dVal /= 10
